...
def abrir_averias():
    ...
    columnas_necesarias = ["LINEA", "EMPLAZAMIENTO", "OT", "DESCRIPCIÓN DE LA FALLA", "ACTIVO", "CAT ", "TIPO", "FECHA HORA INFORME", "ESTADO SICE", "Semana", "TIPO SICE", "SISTEMA"]
...
#############################
    ...
    tabla_adherencia = doc.add_paragraph("\n    Tabla N°10– Análisis de fallas operacionales mes de " + Nombre_mes_actual() + ".")
    tabla_adherencia.runs[0].font.name = 'Calibri'
    tabla_adherencia.runs[0].font.size = Pt(9.5)
    tabla_adherencia.runs[0].font.bold = False
    tabla_adherencia.alignment = 1
    ...
    mantenimiento3 = doc.add_paragraph("\n      Durante la Semana " + semanas_transcurridas() + " no se informan averías escaladas a proveedores para análisis y solución.")
    mantenimiento3.runs[0].font.name = 'Calibri'
    mantenimiento3.runs[0].font.size = Pt(9.5)

    # Crear la tabla con encabezado principal y columnas
    num_columnas = 9  # Número de columnas según la imagen
    ...
    sub_encabezado8 = ['N°', 'Línea', 'Estación', 'OT', 'Descripción de la Falla', 'Categoría', 'Estado', 'Semana', 'Proveedor']
    ...
    tabla_adherencia = doc.add_paragraph('Tabla N°11– Análisis Falla Operacionales Semana ' + semanas_transcurridas() + ".")
    tabla_adherencia.runs[0].font.name = 'Calibri'
    tabla_adherencia.runs[0].font.size = Pt(9.5)
    tabla_adherencia.runs[0].font.bold = False
    tabla_adherencia.alignment = 1
    ...
    mantenimiento3 = doc.add_paragraph("\n      A continuación, se detallan las componentes extraordinarias presentadas en el mes.")
    mantenimiento3.runs[0].font.name = 'Calibri'
    mantenimiento3.runs[0].font.size = Pt(9.5)
    ...
    try:
        if not df_averias.empty:
            ...
            df_filtro2.columns = ["LINEA", "EMPLAZAMIENTO", "OT", "DESCRIPCIÓN DE LA FALLA",
                                  "SISTEMA", "Autorizado", "Código valor", "Fecha de solución", "Semana"]

            # Formatear la columna 'FECHA HORA INFORME' para que solo contenga la fecha
            if 'Fecha de solución' in df_filtro2.columns:
                df_filtro2['Fecha de solución'] = pd.to_datetime(df_filtro2['Fecha de solución'], errors='coerce').dt.date
            ...
            for i, row in enumerate(df_filtro2.itertuples(index=False), start=2):  # start=2 para saltar la fila de título
                ...
                for j, value in enumerate(row, start=1):
                    cell = tabla10.cell(i, j)
                    column_name = df_filtro2.columns[j - 1]  # Ajustar índice de la columna

                    # Formatear la fecha
                    if column_name == "Fecha de solución" and isinstance(value, (datetime, str)):
                        try:
                            fecha = pd.to_datetime(value).date()
                            cell.text = fecha.strftime("%d-%m-%Y")
                        except:
                    ...
                    if column_name == "DESCRIPCIÓN DE LA FALLA":
                        cell.width = descripcion_width
                    elif column_name == "Fecha de solución":
                        cell.width = Inches(1.5)
    ...
    plan_mantenimiento = doc.add_heading("5     Gestión INFOR ", 1)
    plan_mantenimiento.runs[0].font.name = 'Calibri'
    plan_mantenimiento.runs[0].font.size = Pt(10)
    plan_mantenimiento.runs[0].font.color.rgb = RGBColor(0, 0, 0)  # Negro

    mantenimiento3 = doc.add_paragraph("\n      A continuación, se muestra reporte del estado de las órdenes de trabajo.")
    mantenimiento3.runs[0].font.name = 'Calibri'
    mantenimiento3.runs[0].font.size = Pt(9.5)
    ...
    sub_encabezado8 = ['Estado', 'En Curso', 'Emitida', 'Pendiente', 'En Verificación', 'Completada', 'Cerrada']
    ...
    datos = [
        ...
        ["Histórico " + str(datetime.now().year), "0", "0", "0", "0", "0", "0"]]
    ...
    tabla_adherencia = doc.add_paragraph('Tabla N°13 – Tabla de gestión de INFOR Semana ' + Nombre_mes_actual() + ".")
    tabla_adherencia.runs[0].font.name = 'Calibri'
    tabla_adherencia.runs[0].font.size = Pt(9.5)
    tabla_adherencia.runs[0].font.bold = False
    tabla_adherencia.alignment = 1
    ...
    mantenimiento3 = doc.add_paragraph("\n      A continuación, los hallazgos de seguridad de la semana en estudio.")
    mantenimiento3.runs[0].font.name = 'Calibri'
    mantenimiento3.runs[0].font.size = Pt(9.5)

    # Crear la tabla con encabezado principal y columnas
    num_columnas = 3  # Número de columnas según la imagen
    tabla13 = doc.add_table(rows=2, cols=num_columnas)
    tabla13.style = 'Table Grid'

    # Contenido de las celdas
    sub_encabezado8 = ['Hallazgo', 'Plan de acción', 'Fecha de solución']
    ...
    tabla_adherencia = doc.add_paragraph('Tabla N°14 – Hallazgos de seguridad Semana ' + semanas_transcurridas() + ".")
    tabla_adherencia.runs[0].font.name = 'Calibri'
    tabla_adherencia.runs[0].font.size = Pt(9.5)
    tabla_adherencia.runs[0].font.bold = False
    tabla_adherencia.alignment = 1
